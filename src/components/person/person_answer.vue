...
<template lang="html">
  <div class="answerOrder">
    ...
    <p style="text-align:center;color:#666;font-size:18px;padding-top:30px;border-top:1px solid #ccc;" v-show="!orderLength">暂无更多项目</p>
  </div>
</template>
...
export default {
  ...
  methods:{
    ...
    applyOrder(index){//申请项目
      if(!this.userMes.engineerVO){
        this.$confirm('当前操作需要进行登录,是否跳转至登录页？', '提示', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
        }).then(()=>{
          this.$router.push('/engineer/loginIn');
        }).catch(()=>{
          console.log('已取消')
        })
      }else if(this.userMes.engineerVO.state==0||this.userMes.engineerVO.state==1){
        this.$confirm('当前操作需要进行工程师认证,是否跳转至认证页？', '提示', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
        }).then(()=>{
          this.$router.push('/mine/personMes');
        }).catch(()=>{
          console.log('已取消')
        })
        ...
        this.$ajax.post(this.url+'/mission/saveMissionRecord',formdata).then((res)=>{
          if(res.data.code==0){
